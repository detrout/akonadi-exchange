...
bool MapiAppointmentException::preparePayload()
{
    ...
    if (m_pattern->WriterVersion2 >= 0x00003009) {
        changeHighlight = ee->ChangeHighlight.Value;
        description += i18n("\n    ChangeHighlight %1", changeHighlight);
    }
    if (overrideFlags & ARO_SUBJECT) {
        title.setUtf16(static_cast<const ushort *>(ee->Subject),
		       static_cast<int>(strlen_m_ext(ee->Subject, CH_UTF8, CH_UTF16LE)));
        description += i18n("\n    Subject %1", title);
    }
    if (overrideFlags & ARO_LOCATION) {
        location.setUtf16(static_cast<const char * >(ee->Location),
			  static_cast<int>(strlen_m_ext(ee->Location, CH_UTF8, CH_UTF16LE)));
        description += i18n("\n    Location %1", location);
    }
#else
    ...
    if (overrideFlags & ARO_LOCATION) {
        location = QString::fromLatin1((const char *)e->Location.subjectMsg.msg, e->Location.subjectMsg.msgLength2);
        description += i18n("\n    Location %1", location);
    }
#endif
